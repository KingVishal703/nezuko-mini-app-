...
function daily(){
 ref.once("value").then(s=>{
  ...
  if(now-d.lastDaily<86400000) return alert("Already claimed");
...
function watchAd(){
 ref.once("value").then(s=>{
  ...
  if(now-d.lastAd<60000) return alert("Wait 60s");
...
function withdraw(){
 ...
 ref.once("value").then(s=>{
  ...
  if(d.balance<CONFIG.withdrawMin){
   alert("Minimum ₹"+CONFIG.withdrawMin);
   return;
  }
